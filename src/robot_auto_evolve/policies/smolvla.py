...
class _Session:
    policy_seed: int
    task_id: str
    adapter: SmolVLARoboCerebraAdapter
    call_index: int = 0
...
def _require_clean(source: Path) -> None:
    ...
    if dirty:
        raise RuntimeError(f"LeRobot source working tree is dirty: {dirty.splitlines()[0]}")
...
def _validate_checkpoint(config: Any, snapshot: Path) -> None:
    inputs = {key: tuple(value.shape) for key, value in config.input_features.items()}
    expected = {
        "observation.state": (6,),
        "observation.images.camera1": (3, 256, 256),
        "observation.images.camera2": (3, 256, 256),
        "observation.images.camera3": (3, 256, 256),
        "observation.images.empty_camera_0": (3, 480, 640),
    }
    outputs = {key: tuple(value.shape) for key, value in config.output_features.items()}
    if inputs != expected or outputs != {"action": (7,)}:
        raise RuntimeError("SmolVLA RoboCerebra checkpoint feature schema mismatch")
    actual = {
    ...
    if actual != expected_runtime or config.vlm_model_name != BASE_MODEL_ID:
        raise RuntimeError(f"SmolVLA RoboCerebra checkpoint runtime schema mismatch: {actual}")
    contract = checkpoint_state_contract(
        snapshot / "config.json",
        snapshot / "policy_preprocessor_step_5_normalizer_processor.safetensors",
    )
    if contract != {"declared_width": 6, "normalizer_width": 8, "runtime_width": 8}:
        raise RuntimeError("SmolVLA RoboCerebra state contract changed")
...
class SmolVLARoboCerebraPolicyBackend:
    def __init__(self, config: PolicyServiceConfig, source_root: str | Path, device: str) -> None:
        ...
        if not (source / "src" / "lerobot" / "policies" / "smolvla").is_dir() or _head(source) != config.route.source_commit:
        ...
        if not _inside(lerobot, source) or transformers.__version__ != "4.53.3":
            raise RuntimeError("SmolVLA imported runtime differs from the pinned environment")
        if device != "cuda:0" or torch.cuda.device_count() != 1:
            raise RuntimeError("SmolVLA replica requires exactly one visible GPU as cuda:0")
        ...
        if snapshot.name != config.route.revision or base.name != BASE_MODEL_REVISION:
            raise RuntimeError("SmolVLA cached artifact revision mismatch")
        if any(not (base / name).is_file() for name in BASE_FILES):
            raise RuntimeError("SmolVLA base snapshot is incomplete")
        ...
        _validate_checkpoint(policy_config, snapshot)
        policy_config.device = str(self.device)
        policy_config.vlm_model_name = str(base)
        policy_config.load_vlm_weights = True
        self.model = modeling.SmolVLAPolicy.from_pretrained(
            snapshot,
        ...
        self.preprocess = pipeline.PolicyProcessorPipeline.from_pretrained(
            ...
            overrides={
                ...
                "tokenizer_processor": {"tokenizer_name": str(base)},
            },
    ...
    def smoke(self) -> None:
        task, request = synthetic_request(self.config.route.name)
        self.reset({"policy_seed": 0, "task_id": task}, request.session_id, "startup-smoke-reset")
        ...
        if result["execution_count"] != 1 or result["start_step"] != 0:
            raise RuntimeError("SmolVLA startup smoke returned an invalid action")
    ...
    def _infer(self, payload: Mapping[str, Any], seed: int) -> np.ndarray:
        frame = {
            ...
            "observation.images.wrist_image": np.asarray(payload["observation.images.wrist_image"], dtype=np.uint8),
        ...
        if result.shape != (MODEL_HORIZON, 7) or not np.isfinite(result).all():
            raise RuntimeError(f"SmolVLA returned invalid action shape {result.shape}")

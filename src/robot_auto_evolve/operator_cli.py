...
def build_parser() -> argparse.ArgumentParser:
    ...
    route.add_argument("--reuse-agent", action=argparse.BooleanOptionalAction, default=True, help="W3-C2 (DEFAULT ON): reuse a long-lived agent worker per (worker thread, policy replica) across episodes instead of spawning one per episode. Byte-equivalent (validated); pass --no-reuse-agent to disable.")
    route.add_argument("--reuse-sim", action=argparse.BooleanOptionalAction, default=True, help="W3-C3 (DEFAULT ON): reuse a long-lived simulator subprocess per (worker thread, render GPU) across episodes (fresh env each episode). Byte-equivalent on SimplerEnv/RoboCerebra/VLABench/LIBERO-Pro; AUTO-DISABLED for RoboCasa365 (its scene randomization diverges under reuse). Pass --no-reuse-sim to disable.")
